...
def run(args):
    ...
    cmd = [args.ffmpeg_bin, "-f", "concat", "-safe", "0", "-i", f"{args.output}/{args.video_name}_vidlist.txt", "-c", "copy", f"{video_save_path}", "-y"]
    print(" ".join(cmd))
    subprocess.call(cmd)
...
def main_upscale():
    ...
    parser.add_argument("-i", "--input", type=str, default="inputs/wav2lip_out/output.mp4", help="Input video, image or folder")
    parser.add_argument(
        "-n",
        "--model_name",
        type=str,
    ...
    parser.add_argument("-o", "--output", type=str, default="result", help="Output folder")
    parser.add_argument(
        "-dn",
        "--denoise_strength",
        type=float,
